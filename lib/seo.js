...
function pageCopy(pageId, locale) {
  const es = {
    home: {
      title: 'Fundez — Servicios premium a domicilio en Santiago',
      description: 'Técnicos verificados a domicilio en Santiago: gasfitería, electricidad, cerrajería y más. Seguimiento en tiempo real, pago con tarjeta y Pasaporte Hogar digital.',
  ...
  const en = {
    home: {
      title: 'Fundez — Premium home services in Santiago',
      description: 'Verified technicians at home in Santiago: plumbing, electrical, locksmithing and more. Live tracking, card payments and digital Home Passport.',
...
function buildJsonLd(pageId, locale) {
  ...
  const service = {
    '@context': 'https://schema.org',
    '@type': 'Service',
    name: locale === 'en' ? 'Premium home services' : 'Servicios premium a domicilio',
...
function robotsTxt() {
  const siteUrl = getSiteUrl();
  const disallow = DISALLOW_PATHS.map((p) => `Disallow: ${p}`).join('\n');
...
> Premium verified home services platform in Santiago, Chile — for households and for companies, technicians and professionals who want to earn as partners.
...
> Plataforma chilena de servicios premium a domicilio en Santiago — para hogares y para empresas, técnicos y profesionales que quieren generar ingresos como socios.
