...
async function createOrder({ request, service, baseUrl }) {
  ...
  const res = await fetch(`${getApiBase()}/v2/checkout/orders`, {
    ...
    body: JSON.stringify({
      intent: 'CAPTURE',
      purchase_units: [{
        reference_id: String(request.id).slice(0, 256),
        description: `Fundez — Visita técnica: ${service?.name || 'Servicio'}`,
      ...
      application_context: {
        ...
        user_action: 'PAY_NOW',
        return_url: `${baseUrl}/pagos/paypal/retorno?ref=${encodeURIComponent(request.id)}`,
        cancel_url: `${baseUrl}/pagos/error?ref=${encodeURIComponent(request.id)}&motivo=cancelado`
      }
    })
  });
