...
function getSnapshotPath(backup) {
  return path.join(backup.folderPath, 'snapshot.json');
}

function readSnapshot(backupId) {
...
async function restoreBackup(store, backupId, { triggeredBy = 'admin', restoreUploads = true } = {}) {
  ...
  if (restoreUploads && backup.includesUploads) {
    uploadsRestored = restoreUploadsFromBackup(backup);
  }
  ...
  return {
    ...
    stats,
    uploadsRestored,
    appVersion: backup.appVersion || snapshot.appVersion
  };
}
