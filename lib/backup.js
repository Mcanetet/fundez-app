...
function saveConfig(updates) {
  ...
  if (backupStore.isAvailable()) {
    backupStore.saveConfig(next).catch((err) => {
      console.error('[Fundez Backup] Error guardando config en MySQL:', err.message);
    });
  }
  return next;
}
...
function persistBackupRecord(manifest, snapshot, folderName) {
  ...
  backupStore.saveBackup({
    ...
    console.error('[Fundez Backup] Error guardando backup en MySQL:', err.message);
  });
}
...
function importSnapshotFile(snapshot, triggeredBy = 'admin') {
  ...
  persistBackupRecord(manifest, snapshot, folderName);
...
async function restoreFromSnapshotData(store, snapshot, { triggeredBy = 'admin', restoreUploads = false, saveImport = true } = {}) {
  ...
  const preRestore = createBackup(store, 'pre-restore', triggeredBy);
  const stats = await store.importDataSnapshot(snapshot);

  let imported = null;
  if (saveImport) {
    imported = importSnapshotFile(snapshot, triggeredBy);
  }
  ...
  return {
    ...
    appVersion: snapshot.appVersion || preRestore.manifest.appVersion,
...
function restoreUploadsFromBackup(backup) {
  ...
  const src = path.join(backup.folderPath, 'uploads');
  if (!fs.existsSync(src)) return false;
  if (fs.existsSync(UPLOADS_ROOT)) {
    fs.rmSync(UPLOADS_ROOT, { recursive: true, force: true });
  }
  copyDirRecursive(src, UPLOADS_ROOT);
  return true;
}
