...
function buildSystemPrompt(config, knowledgeContext, serviceName) {
  return [
    ...
    'Si el tema es de pagos/cobros/facturación, responde y termina con [DERIVAR_PAGOS] en una línea aparte. No menciones WhatsApp.',
    '',
...
async function generateAlandReply({ appStore, conversation, userMessage, config }) {
  ...
  if (escalatePayments && !/pago|cobro|admin|equipo|factur/i.test(reply)) {
    reply += '\n\nDerivé tu caso al equipo de pagos de Fundez. Revisarán tu consulta desde la plataforma; no necesitas escribir por WhatsApp.';
...
async function escalatePaymentIssue({ appStore, conversation, io, user, userMessage }) {
  ...
  const handoff = await alandStore.addMessage({
    ...
    senderType: 'aland',
    senderName: 'Aland IA',
    body: 'Ya avisamos al equipo de Fundez. Ellos revisarán tu caso en la plataforma. Mientras tanto puedes seguir escribiendo aquí si quieres agregar detalle.',
