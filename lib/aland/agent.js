...
async function syncKnowledgeFromApp(appStore) {
  ...
  const pricingBlock = [
    `Visita base: ${formatCLP(pricing.visitBasePrice || pricing.visitPrice)}`,
    `Recargo tarjeta: ${pricing.cardSurchargePercent || 0}%`,
    `Transferencia habilitada: ${pricing.transferEnabled ? 'Sí' : 'No'}`,
    `Tiers de urgencia: ${(pricing.urgencyTiers || []).map((t) => `${t.label} (${t.adjustmentPercent > 0 ? '+' : ''}${t.adjustmentPercent || 0}%)`).join(', ')}`
  ].join('\n');
