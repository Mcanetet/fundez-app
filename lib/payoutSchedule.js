...
/** Siguiente día hábil en Chile (salta fin de semana y feriados). */
function nextBusinessDayIso(from = new Date()) {
  let day = addDays(chileParts(from), 1);
  for (let i = 0; i < 14; i += 1) {
    const weekday = new Date(Date.UTC(day.year, day.month - 1, day.day)).getUTCDay();
    if (weekday !== 0 && weekday !== 6 && !isChileHoliday(day)) {
      return isoDate(day);
    }
    day = addDays(day, 1);
  }
